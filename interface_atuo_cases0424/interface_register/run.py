...
import requests
import os
from interface_atuo_cases0424.public.config import config
from interface_atuo_cases0424.public.readExcel import readExcel
from interface_atuo_cases0424.public.writeExcel import writeExcel

url = config ().read_config (os.path.dirname (os.getcwd ()) + '/conf/' + 'http.conf', 'HTTP', 'url')
#print (url)
h = readExcel (os.path.dirname (os.getcwd ()) + '/test_data/' + 'testcases.xlsx', 'test_cases')
data=h.read_Excel()
...
try:
    ...
    for i in range(len(data)):
        request=requests.get(url,eval(data[i]['data']))
        result=request.json()
        #print (result)
        #print (data[i]['data'])
        #写入excel数据
