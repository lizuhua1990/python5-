...
class massageMail:
    ...
    def Message(self):
        data=massageMail().read_config(os.path.dirname (os.getcwd ()) + '/conf/' + 'smtpconf.conf','STMPMASSAGEMIAL','config')
        print(data)
        # Python 发送带附件的邮件
        ...
        att1 = MIMEText (open (os.path.dirname(os.getcwd ())+ '/interface_register/'+ '2018-05-08_16_48_14python5.html', 'rb').read (), 'base64', 'utf-8')
        ...
        try:
            ...
            s.sendmail (mail_user, receivers.split(',')+ acc.split(','), message.as_string ())
            print ("邮件发送成功")
